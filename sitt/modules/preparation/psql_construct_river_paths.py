...
from shapely import LineString, MultiLineString, Point, wkb, get_parts, get_coordinates, prepare, destroy_prepared, \
    line_merge, delaunay_triangles, STRtree, contains, overlaps, intersection
from sqlalchemy import create_engine, Table, Column, select, literal_column, func, text, insert
...
class PsqlConstructRiverPaths(PSQLBase):
    ...
    def run(self, config: Configuration, context: Context) -> Context:
        ...
        t = Table("parts", self.metadata_obj, Column("geom"), schema="water_wip")

        for idx, water_body in groups.items():
            geom = water_body['geom']
            prepare(geom)
            ...
            for part in parts:
                ...
                if contains(geom, part):
                    stmt = insert(t).values(geom=literal_column("'SRID=" + str(self.crs_no) + ";" + str(part) + "'"))
                    compiled_stmt = stmt.compile(compile_kwargs={'literal_binds': True})
                    self.conn.execute(compiled_stmt)
                    print(c / total, compiled_stmt)
                    self.conn.commit()
                elif overlaps(geom, part):
                    parts = get_parts(intersection(geom, part))
                    for p in parts:
                        if p.geom_type == 'Polygon':
                            stmt = insert(t).values(
                                geom=literal_column("'SRID=" + str(self.crs_no) + ";" + str(p) + "'"))
                            compiled_stmt = stmt.compile(compile_kwargs={'literal_binds': True})
                            self.conn.execute(compiled_stmt)
                            print(c / total, compiled_stmt)
                            self.conn.commit()
            ...
            # to_consider = set()
            #
            # # split the water body into triangles and put them into index
            # tree = STRtree(get_parts(delaunay_triangles(geom)))
            #
            # # first point to check
            # idx = tree.nearest(Point(12.97690006, 46.66985369))
            #
            # to_consider.add(idx)
            #
            ...
            #     stmt = insert(t).values(geom=literal_column("'SRID=" + str(self.crs_no) + ";" + str(entity) + "'"))
            #     compiled_stmt = stmt.compile(compile_kwargs={'literal_binds': True})
            #     self.conn.execute(compiled_stmt)
            #     self.conn.commit()
            #     print(compiled_stmt)
            #     # add to entries considered
            #     already_considered.add(idx)
            #
            #     # get neighbors
            #     connections = tree.query(entity, 'touches')
            #     for i in connections:
            #         if i not in already_considered:
            #             already_considered.add(i)
            #             shape = tree.geometries.take(i)
            #             if contains(geom, shape):
            #                 # print("INSERT INTO water_wip.parts VALUES ('SRID=4326;" + str(shape) + "');")
            #                 to_consider.add(i)
            #
            #     sys.exit(-1)
    ...
    #
    #
    #     logger.info(f"Getting approximate medial axis for river system {idx} - this can take a very long time...")
    #     exit(0)
    ...
    #
    #     file = open("axis.pickle", 'wb')
    #     pickle.dump(ideal_axis, file)
    #     print("Ok")
    #     exit(0)
    #
